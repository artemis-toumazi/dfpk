...
#' @export
pkpop <-
function(y, auc, doses, x, theta, prob = 0.9, options = list(nchains = 4, niter = 4000, nadapt = 0.8), betapriors = c(10000, 10, 5), thetaL=NULL, p0 = NULL, L = NULL, 
         deltaAUC = NULL){
    ...
    # For STAN model
    data_s <- list(N=num,auc=log(auc),dose=dose1, beta0 = betapriors[1])
    ...
    mu1 <- cbind(rep(1,num), mu[x])
    
    # For STAN model
    data_s <- list(N=num,y=y,dose=mu1, beta3mean = betapriors[2], beta4mean = betapriors[3])
